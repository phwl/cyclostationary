...
achieves a mean error of $3.0\%$ on populated harmonics, against $8$--$12\%$
for the comb estimator, reflecting the latter's reliance on far fewer
samples. Error is largest for the weakest, highest-order harmonics in both
methods.
...
\vspace{-6pt}
\caption{Relative profile-magnitude error by harmonic index, comparing
sparse SCD reconstruction and the proposed comb estimator against the
dense reference.}
\label{fig:error}
\end{figure}
...
the roughly threefold accuracy gap in Section 4.4 reflects this difference
